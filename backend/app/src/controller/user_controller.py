...
async def register(
    ...
    logger.info("开始注册用户", client_ip=client_ip)
    resp = await user_service.create_user(user_data, client_ip)
    logger.info("用户注册成功", client_ip=client_ip)
    return success_200(
        data=resp,
        message="用户注册成功",
